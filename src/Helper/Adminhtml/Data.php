...
/**
 * @category ScandiPWA
 * @package ScandiPWA\MenuOrganizer\Helper\Adminhtml
...
class Data extends \Magento\Captcha\Helper\Data
{
     ...
     * Available event scandipwa_menuorganizer_menu_get_available_statuses to customize statuses.
     *
     * @return array
     */
     ...
     *
     * @return \ScandiPWA\MenuOrganizer\Model\Menu $model
     */
    public function initMenu($idFieldName = 'menu_id')
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();

        $model = $objectManager->create('ScandiPWA\MenuOrganizer\Model\Menu');

        if ($menuId = (int)$this->_getRequest()->getParam($idFieldName, false)) {
            $model->load($menuId);
        }

        if (!$this->_coreRegistry->registry('scandipwa_menuorganizer_menu')) {
    ...
    {
        if ($registryItem = $this->_coreRegistry->registry('scandipwa_menuorganizer_item')) {
            return $registryItem;
        }

        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();

        $model = $objectManager->create('ScandiPWA\MenuOrganizer\Model\Item');

        if ($menuId = (int)$this->_getRequest()->getParam($idFieldName)) {
            $model->load($menuId);
        }

        $this->_coreRegistry->register('scandipwa_menuorganizer_item', $model);
