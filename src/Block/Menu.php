<?php
namespace ScandiPWA\MenuOrganizer\Block;
...
/**
 * @category ScandiPWA
 * @package ScandiPWA\MenuOrganizer\Block
...
class Menu extends Template implements IdentityInterface
{
    ...
    protected $_menuItemCollectionClass = \ScandiPWA\MenuOrganizer\Model\ResourceModel\Item\Collection::class;
    ...
    protected function _getHtml(Node $menuTree, $childrenWrapClass, $limit, $colBrakes = [])
    {
        ...
        foreach ($children as $child) {
            ...
            $html .= '<a href="' . $child->getFullUrl() . '" ' . $outermostClassCode . $child->getType() . '>';

            if ($child->getIcon()) {
                $html .= sprintf('<img src="%s" alt="%s" />',
                    $this->_urlBuilder->getDirectUrl($child->getIcon(),
                    ['_type' => UrlInterface::URL_TYPE_MEDIA]), $child->getIconAlt()
                );
            }

            $html .= '<span>' . $this->escapeHtml($child->getTitle()) . '</span></a>'
                . $this->_addSubMenu(
                    $child,
                    $childLevel,
                    $childrenWrapClass,
    ...
    protected function _getMenuItemClasses(Node $item)
    {
        ...
        if ($item->getItemClass()) {
            $classes[] = $item->getItemClass();
        }
    ...
    {
        return $this->_menuModel->getCssClass();
    }
    ...
    protected function _getMenuItemCollection()
    {
        if (!$this->_menuItemCollection) {
            ...
            /**
             * @var $collection \ScandiPWA\MenuOrganizer\Model\ResourceModel\Item\Collection
             */
    ...
    /**
     * @return bool | \ScandiPWA\MenuOrganizer\Model\ResourceModel\Menu
     */
